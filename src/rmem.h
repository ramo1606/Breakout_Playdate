...
#include "memory.h"         // Required for: pd_malloc(), pd_calloc(), pd_free()
...
MemPool CreateMemPool(const size_t size)
{
    ...
    else
    {
        ...
        uint8_t *const restrict buf = pd_malloc(size*sizeof *buf);
...
ObjPool CreateObjPool(const size_t objsize, const size_t len)
{
    ...
    else
    {
        ...
        uint8_t *const restrict buf = pd_calloc(len, aligned_size);
...
BiStack CreateBiStack(const size_t len)
{
    ...
    uint8_t *const buf = pd_malloc(len*sizeof *buf);
